...
Direct commits to the main branch are not permitted. All changes must be proposed through a Pull Request, which is reviewed and validated before being merged. Feature branches should be a single logical change and named descriptively. Branches should be kept reasonably up to date with main to reduce merge conflicts.
